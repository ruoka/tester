...
**`list` vs build:** `list` is the module/source graph — each `unit` carries `imports[]`, `level`, and naming from the scanned suffixes; the same inventory is also written as `graph.json` at the project root. A dependency cycle aborts `list` and `build` with a thrown message on stderr (`Cyclic dependency detected between units: …`), not a structured inventory event. Missing or stale BMIs are build-only: read `compile_end.rebuild` (`own_bmi_missing`, `bmi_stale`, `dependency_bmi_stale`, …), not `list`.

**Compile `rebuild_reason` kinds:** `not_in_cache` (first seen), `source_stale` (edited), `header_stale` (an `#include`d header in the project tree is newer than the relevant object/BMI, resolved from the compiler depfile — the header is in `rebuild.trigger_path`), `header_missing` (a project header named by the depfile is missing or unreadable — the header path is in `rebuild.trigger_path`), `depfile_unusable` (the compiler `.d` is missing, unreadable or malformed, so header freshness is unknown — the `.d` path is in `rebuild.trigger_path`), `object_missing`, `object_stale` (object older than the cached source timestamp, or — for modular units including `std` — older than its own BMI), `own_bmi_missing`, `own_bmi_stale`, `bmi_stale`, `dependency_bmi_stale`, `profile_change` (see `profile_changed`; `rebuild.see_event` is `"profile_changed"`). Module name is in `rebuild.module`, not encoded in the reason string. For modular units, `object_missing` / `object_stale` are decided only after own-PCM, import, and textual-header freshness (two-phase may reuse the BMI for `pcm → .o`; a stale BMI input re-precompiles instead).
...
| Area | Rule |
|------|------|
| **Subprocesses** | **`posix_spawn` + `waitpid`** via **`cb::process::runner::invoke_shell(argv)`** — the sole process boundary (compile, link, test_runner). Still go through `/bin/sh -c` so capture redirects stay shell syntax. Apple's libc serializes `std::system`, which caps parallel compiles on macOS; `posix_spawn` does not. No `popen`, and no ad-hoc `fork` / `execve` outside that class. |
...
Do not add new POSIX-only paths when `process::runner` + file-read or pure stdlib code suffices.
...
// shell command: quote each arg, then join (non-empty argv — process::runner contract)
return argv
...
**Subprocess I/O** — see [Implementation policy](#implementation-policy-standard-c-only) above. All child commands go through `cb::process::runner::invoke_shell(argv)` (`posix_spawn` of `/bin/sh -c`); the runner owns wait-status decoding, diagnostics capture and command-failure composition. Probes/self-tests use stamp/temp file + `std::ifstream`. Test env uses POSIX `setenv` (copies values) before invoking the runner for `TESTER_CONFIG` / `TESTER_PARENT_RUN_ID` — not shell env prefixes. Never `popen` or ad-hoc `fork` / `execve`.

**CB flag argv** — store toolchain flags as `string_list` (`compile_flags`, `link_flags`, `cpp_flags`, `module_flags`); argv builders extend lists with `append_range` (literal groups as `string_list{…}`). Convert external flag **text** only at boundaries through `cb::flags::codec`: `cb::cli::parser` calls `parse` for `--compile-flags` / `--link-flags`, while `cb::cache::profile` uses the same codec to serialize and diff `compile`/`cpp` fields. `serialize` uses `views::join_with(' ')` + `ranges::to<std::string>`; `parse` privately normalizes whitespace, then uses `views::split(' ')` + `filter` non-empty + `ranges::to<string_list>`. The pair is symmetric for CB's profile format; it is not full POSIX shell word-splitting (C++26 `split_when` would cover predicate delimiters without the normalization step).

**`cb.c++` namespaces** — each same-file subsystem has an explicit ownership boundary: `cb::source::translation_unit` models scan identity (path, module, imports, kind) only — not object/BMI/executable paths; `cb::source::scanner` owns collection, exclusion, lexical cleaning, dependency edges and topological order, and `cb::flags::codec` owns flag-list text conversion. `cb::artifacts` owns per-unit object/BMI/executable path records (`of_unit` / `index`). `cb::toolchain` owns module/linkage and target-artifact conventions plus `driver_state` and the concrete `clang_driver` (constrained by `toolchain::driver`, which includes `state() -> driver_state`): LLVM discovery and identity, Clang/libc++ flags, semantic compile-step callbacks and compile/link argv construction; `module_provider` is the driver-facing view of a modular interface (module, imports, BMI path). A second driver model means templating `build_system` — the member stays concrete today. `cb::build_tree::paths` owns the configured build tree, source-to-artifact naming, and depfile/diagnostic sidecars; it uses `std::filesystem::path` internally and consumes the concrete toolchain's non-owning extension conventions. `cb::cache::profile` owns profile serialization, keys and diffs, `cb::cache::link_signature` is the symmetric serialize/parse/compare type for executable stamps, `cb::cache::rebuild_reason_for_standard_module` / `rebuild_reason_for_link` are the std and link oracles (no project artifact state), and `cb::cache::analyzer` is the memoized project-TU freshness oracle over the artifact index. Cache classes compose `cb::cache::storage_file` for path, first-line, atomic replacement, and invalidation operations; `object_store`, `link_store`, and `standard_module_store` own persistence (and std hydrate/publish); `compiler_stamp_store` owns the version stamp file. `cb::process::runner` is the sole process boundary and privately owns argv joining, wait-status decoding, diagnostics capture, and failed-command messages; `cb::process::shell_quote` is the shared quote helper (runner joins and `clang_driver::lookup_argv` probes). `cb::execution::run_workers` owns the common thread-group spawn/join lifecycle; `worker_count` / `failure_latch` are shared by `worker_pool` (bounded fixed-range jobs) and `build_system::compile_schedule` (edge-driven readiness queue; two-phase modular rebuilds publish after `--precompile` and enqueue a separate `object_followup` job so a worker can claim an importer while `pcm→.o` runs later). The RAII `build_scope`, `compile_scope`, `link_scope`, and `test_scope` live under `cb::output` beside the observer contracts whose event pairs they guarantee. `cb::cli::parser` / `options` own argv parsing, test-runner forwarding, and projection into `build_system::settings`; `known_tokens` is the single vocabulary that both classifies and dispatches parse actions (`token_owner` is `cb` or `test_runner` — CB-owned flags the runner also understands are resynthesized by `runner_arguments`, not dual-forwarded; `cb test --help` is CB usage). `cb::scanned_project` owns one scan's `units` plus the derived `artifacts` / `by_unit` / `interfaces` views (rebuilt together by `scan_and_order`); `cb::build_system` remains the settings-driven orchestration boundary: it owns that bundle, a post-scan `cache::analyzer` referencing its artifact index, build-step execution, scheduling, reporting and list/test actions; nested `cache_admin` (via `caches()`) is constructed from `paths`, `driver`, and an object-cache profile invocable — store factories, `status` / `invalidate` / `clean`, and `evict`; the toolchain probe for `cache status` runs at the call site via `ensure_toolchain_profile`. `clean_tests` stays on `build_system` (scan + which TUs are tests + artefact deletion) and then asks `caches().evict`. Orchestration asks `caches()` for stores, the analyzer for rebuild reasons, `build_tree::paths` for artifact locations and `clang_driver` for compile/link argv. Depfile and compile/link log suffixes live with the other project naming constants at the top of `cb.c++`. `main` installs the observer selected by parsed CLI options, while activation and finalization happen through the observer contract. Shared profile-diff and source-inventory models live under `cb::output`; channel-specific formatting/serialization stays in `cb::output::console::observer` and `cb::output::jsonl::observer`. `cb::detail` is limited to cross-subsystem filesystem/path primitives: `join_dir`, `path_under_dir`, `path_at_or_under_dir`, `path_at_or_under_root`, `canonical_path`, `write_atomic_file`, and `remove_if_exists`.
