...
**`cb.c++` namespaces** — each same-file subsystem has an explicit ownership boundary: `cb::source::translation_unit` models one source, `cb::source::scanner` owns collection, exclusion, lexical cleaning, dependency edges and topological order, and `cb::flags::codec` owns flag-list text conversion. `cb::toolchain` owns module/linkage and target-artifact conventions plus the concrete `clang_driver`: LLVM discovery and identity, Clang/libc++ flags, semantic compile-step callbacks and compile/link argv construction. `cb::build_tree::paths` owns the configured build tree, source-to-artifact naming, and depfile/diagnostic sidecars; it uses `std::filesystem::path` internally and consumes the concrete toolchain's non-owning extension conventions. `cb::cache::profile` owns profile serialization, keys and diffs, and `cb::cache::analyzer` owns all rebuild-policy oracles (project TU object/BMI/header freshness and depfile parsing, std BMI/object reuse, and link stamp compare). Cache classes compose `cb::cache::storage_file` for path, first-line, atomic replacement, and invalidation operations; `object_store`, `link_store`, and `standard_module_store` own persistence (and std hydrate/publish); `compiler_stamp_store` owns the version stamp file. `cb::process::runner` is the sole process boundary and privately owns argv joining, wait-status decoding, diagnostics capture, and failed-command messages; `cb::process::shell_quote` is the shared quote helper (runner joins and `clang_driver::lookup_argv` probes). `cb::execution::run_workers` owns the common thread-group spawn/join lifecycle; `worker_pool` adds bounded fixed-range claiming and join-before-rethrow failure handling for independent jobs, while the compile scheduler retains its dynamic readiness queue. The RAII `build_scope`, `compile_scope`, `link_scope`, and `test_scope` live under `cb::output` beside the observer contracts whose event pairs they guarantee. `cb::cli::parser` / `options` own argv parsing, test-runner forwarding, and projection into `build_system::settings`. `cb::build_system` remains the settings-driven orchestration boundary: it owns the source graph, module-import closure, cache wiring, build-step execution, scheduling, reporting and list/test actions; it asks `cache::analyzer` for rebuild reasons, `build_tree::paths` for artifact locations and `clang_driver` for compile/link argv. `main` installs the observer selected by parsed CLI options, while activation and finalization happen through the observer contract. Shared profile-diff and source-inventory models live under `cb::output`; channel-specific formatting/serialization stays in `cb::output::console::observer` and `cb::output::jsonl::observer`. `cb::detail` is limited to cross-subsystem filesystem/path primitives: `join_dir`, `path_under_dir`, `path_at_or_under_dir`, `path_at_or_under_root`, `canonical_path`, `write_atomic_file`, and `remove_if_exists`.
