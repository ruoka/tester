...
- **CB maps only the project BMIs each translation unit actually needs.** Compile and
  precompile commands now receive `-fmodule-file=` entries for direct and transitive
  imports (plus an implementation unit's primary interface), rather than every project
  module. `compile_commands.json` uses the same dependency-scoped argv.
