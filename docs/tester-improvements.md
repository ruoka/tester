...
- ✅ CMake + Ninja CI lane (`cmake-ninja-build-and-test` in `.github/workflows/ci.yml`): configure and build [`CMakeLists.txt`](../CMakeLists.txt) for `Debug` and `Release` (fail on any compiler `warning:`), then `[self]` sequentially and at `--jobs=$(nproc)`, the standalone suite, and the `run_tests` target. CMake is pinned via `.github/actions/setup-cmake` because the `import std` gate UUID is version-specific, and the dev container installs the same pin so the third path is runnable where it is documented.
