...
- ✅ Makefile CI lane (`makefile-build-and-test` in `.github/workflows/ci.yml`): `make tests` then the Make-built runner with `--tags='[self]'`, failing the job if the rebuild emits any compiler `warning:`. CB also surfaces warnings from units it compiles (`diagnostics` on successful `compile_end` / `link_end` / `command_end` in failures mode), so Make is no longer the only place `-Wall` noise is visible — it remains the full-rebuild cross-check beside CB.
