...
- ✅ The empty `toolchain::driver` placeholder no longer pretends to be an implemented abstraction. Until compiler discovery and command construction actually move out of `build_system`, the one toolchain-owned type is honestly a `toolchain::module_link_flags` alias.
- ✅ `build_system` has one semantic settings boundary instead of a nine-parameter constructor followed by three setters. `cli::options::build_settings()` projects only build inputs; output selection, actions and test-runner forwarding remain in the CLI layer. The constructor now owns include-path-to-flag conversion, configuration defaults, module mode and worker-limit state, while `main` is reduced to constructing the system from that value.
