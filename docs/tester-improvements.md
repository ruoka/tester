...
- ✅ Class-based subsystem boundaries now divide the single `cb.c++`: `cb::source::translation_unit`
  ...
  `cb::cache::object_store`, `link_store`, `standard_module_store`, and `compiler_stamp` own cache
  paths, maps/signatures, persistence, status/invalidation, and cache-specific reuse decisions.
  `cb::process::runner` owns spawning, status decoding, diagnostics capture and failed-command
  composition; `cb::execution::worker_pool` owns generic bounded independent-job concurrency;
  ...
  primitives shared across those boundaries. No `*.h++` split is required.
