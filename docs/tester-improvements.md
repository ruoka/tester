...
- ✅ Clang command policy has one concrete owner. `toolchain::clang_driver` discovers LLVM, owns compiler identity and Clang/libc++ flags, maps semantic module/BMI pairs to `-fmodule-file=`, and returns compile/link argv. Its compile plans also state which step writes the BMI, so `build_system` can execute and report steps, publish provider readiness, and persist the std profile without encoding one- versus two-phase command sequences. Project compiles take `source::translation_unit` (same file, dependency order); `std` keeps a path-based entry point. Construction is a factory (`clang_driver::make`): the driver emits compiler-lookup and `--version` argv, `process::runner` executes them, and the returned driver is immutable — no injected availability callback. Compiler `--version` text still lives on `build_system` after the stamp probe. There is no virtual base until a working second compiler proves the common interface.
