...
- ✅ Class-based subsystem boundaries now divide the single `cb.c++`: `cb::source::translation_unit`
  ...
  `build_system` coordinates compiler/linker work and event publication without owning cache
  state. `cb::detail` holds only primitives shared across those boundaries. No `*.h++` split is
  required.
