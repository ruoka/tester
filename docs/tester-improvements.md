...
- ✅ Registration kind is explicit: wrappers pass `registration_kind::suite_case` (`test_case` / `scenario`) or `::step` (`section` / `given` / `when` / `then` / `and_*`) plus a role string for display names. Classification no longer searches `function_name()` for `"scenario"` / `"test_case"`. Public API unchanged; a step with no active run still degrades to a scheduled case for compatibility.
- ✅ `runner` owns its tag filter as a `std::string` (not a view into argv / a temporary). Console and JSONL `observer_instance(...)` reconfigure on each call instead of latching the first streams/mode; JUnit already reconfigured via `configure()`.
...
- ❌ No source-level `expression` field via assertion macros — the framework stays macro-free and uses `std::source_location` (file/line/column) today. A future `expression` (or equivalent) would come only from C++26 reflection when that is available and useful, not from preprocessor stringification.
- ✅ Documented event ordering: assertion events stream during execution; `test` records batch at finalize time ([AGENTS.md](../AGENTS.md), [README.md — JSONL](../README.md#assertion-events)).
...
- ✅ Per-translation-unit `compile_end` with `ok`, `duration_ms`, `source_path`, `object_path`, `bmi_path`, `module_name`, `cache_hit`.
...
- ✅ `rebuild_reason` on `compile_end` when `cache_hit:false` (short kind: `not_in_cache`, `source_stale`, `header_stale`, `header_missing`, `depfile_unusable`, `bmi_stale`, `dependency_bmi_stale`, `profile_change`, …).
...
- ✅ JUnit / xUnit XML observer (`tester-junit_observer.c++m`). Additive file sink via `--junit=<path>` (alias `--xunit-xml=`); runs alongside console or `--jsonl` through `observe()` after `select_observer`. CB forwards the flag. Self-tests cover parallel JSONL+JUnit, `<failure>` / `<error>` mapping, empty-filter synthetic failures, and CI-readable `classname` / `name` (project-relative path + case title, not demangled lambdas). Failed cases set `file` / `line` to the assertion call site (relative), not the `test_case` registration line; `classname` is `path:line` for GitHub `test-summary`. Nested BDD `given`/`when`/`then`/`section` steps fold under the parent `scenario`/`test_case` as one Summary row (step checklist in `<system-out>`; JSONL still lists every step). Suite failure/error totals count cases. GitHub Actions emits the reports beside JSONL and publishes gate-suite results with `test-summary/action` (demos stay artifact-only so expected failures do not muddy the job summary).
...
- ✅ Incremental compile cache (`cache::object_store`) and link cache (`cache::link_store`); compile/relink decisions read stable loaded snapshots before workers mutate them, failed workers are joined, and cache indexes use checked temporary-file replacement.
- ✅ Object-cache profile header (`format=cb-object-cache-v4`) with toolchain-neutral fields (`config`, `static_link`, `toolchain_root`, `compiler`, `compiler_signature`, `compiler_version`, `std_module` as `path@size:mtime_ns`, `compile` / `cpp` including `--compile-flags`); invalidates on profile mismatch (`rebuild_reason: "profile_change"`).
...
- ✅ Decoded child status: `exit_code` / `signaled` / `signal` from the `waitpid` wait status, raw value kept as `wait_status`.
...
- ✅ CB / tester implementation policy: standard C++ with `posix_spawn` for subprocesses (Apple's libc serializes `std::system`); stack traces in `test_runner` via `<execinfo.h>` (POSIX exception).
...
- ✅ One atomic file writer instead of three. The former object-cache, executable-cache and std-profile save methods each spelled out the same temp-file / write / close-check / rename / remove-on-error sequence — 87 lines for three callers whose only real differences were what they wrote and what the file was called in the message. `detail::write_atomic_file(path, what, writer)` now owns the sequence for all three cache stores plus `compile_commands.json` and `graph.json`, so each fails identically. Cache classes compose `cache::storage_file` for paths, first-line reads, replacement and invalidation; the executable cache's "empty means delete" branch uses that value's invalidation operation.
...
- ✅ Class-based subsystem boundaries now divide the single `cb.c++`: `cb::source::translation_unit`
  ...
  and diffs, `cb::cache::analyzer` owns all rebuild-policy oracles (project TU object/BMI/header
  freshness and depfile parsing, std reuse, and link stamp compare), and `cb::cli::parser` /
  ...
  fixed-range claiming and failure propagation for independent jobs;
  `cb::output` owns the four RAII lifecycle scopes beside the observer contracts they report
  through. `build_system` coordinates compiler/linker commands without owning cache state,
  process mechanics, generic worker-pool logic or event-pair latches. `cb::detail` holds only
  filesystem/path primitives shared across those boundaries. Toolchain file signatures and
  module-map flag construction are private `build_system` operations. No `*.h++` split is required.
...
| Verb | Purpose |
|------|---------|
| `cache status` | Decode profile header, index entry counts, disk usage (`obj/`, `bmi/`, `cache/`), orphan/stale rows (cache points at missing paths; artifacts with no matching source/TU). JSONL: `cache_status` event. ✅ |
...
| `cache prune` | Garbage-collect artifacts the current module graph no longer references; trim cache index rows for missing paths. Optional `--aggressive` after `profile_change` to drop all `obj/` / `bmi/` under the config. JSONL: `cache_prune_end` with counts. |

**Implementation sketch:** scan current TU graph → expected `{obj, bmi}` set; walk `obj/` and `bmi/`; delete files not in set; reconcile `object-cache.txt`. Reuse `object_cache_profile()` and `cache::profile` field parsing for `status`.
...
- ✅ No `emit_` wrappers remain in `cb.c++`. `emit_compile_start`, `emit_compile_end`, and `emit_link_end` existed because the interface was positional: their whole body was mapping a unit onto unnamed arguments, including two copies of the `is_modular ? bmi_path : {}` projection that could have disagreed. Call sites now notify directly, as the other ~40 already did.
...
- ✅ The build's emit-once latch is a scope, not producer state. `output::build_scope` emits `build_start` on construction and exactly one `build_end` on the way out — success only if `succeeded()` was reached, failure otherwise, including during unwinding. That deleted the `build_phase` enum, the `current_phase` / `phase_started` / `build_end_emitted` members, `emit_failed_build_end`, and the catch-and-rethrow blocks in `build()` and `run_tests()`, which each had to remember the same five-step ritual. The latch lives beside the observer contract, not inside each observer: otherwise every format would reimplement it and could disagree about whether the build ended. The invariant is pinned by the smoke harness (`single_failed_build_end` asserts one `build_end` with `ok:false`, plus the link-failure and test-link-failure cases), and the failed-build event order is unchanged — `build_end` still precedes `cb_error`, because unwinding runs the destructor before the handler in `main`.
...
- ✅ Reading the object cache keeps everything it learned together. `load_object_cache` originally returned the entry map and left two more answers behind in `build_system` members — `object_cache_miss_reason` and `profile_diff` — which the caller collected separately and the analyzer read to attribute a miss to `profile_change`. `cache::object_store` now owns the loaded entry snapshot and optional profile diff; `cache::analyzer` receives the store by const reference, so persistence state no longer escapes into `build_system` and worker reads remain lock-free.
- ✅ How a spawned process ended is one contract type. `shell_result` was declared privately in `cb.c++` although both its members — `process_status` and `diagnostics` — were already contract types, so `command_end` and `test_end` took it apart and passed `ok()`, `status` and `diag` as separate arguments. The `ok` among them was not an independent fact but `status.ok()`, which meant the interface admitted an event claiming success beside a status reporting a signal. `output::process_result` now travels whole: `command_end` is down from six arguments to four, `test_end` from three to two, and `process::runner::invoke_shell` returns the type the event carries. It also gave `diag` a default member initializer, which retired a `-Wmissing-designated-field-initializers` warning at the construction site.
- ✅ Compiles and links pair their events with scopes under `cb::output`, where the event contract lives. `output::compile_scope` emits `compile_start` and exactly one `compile_end`; a modular unit compiles in two steps and either can fail, so each exit path used to carry its own copy of the event and had to agree about the unit, reason and clock. `output::link_scope` is the exit half only — there is no `link_start` — and replaced three copies of `link_end` plus a `linked` bool whose only job was suppressing a duplicate terminal event.
- ✅ Generic independent-job concurrency has one actor. The old compile/link passes each spawned a `jthread` per job behind a `job_gate`, tracked `failed` / `failure` / `failure_mutex`, joined everything, then rethrew the first exception. `execution::worker_pool{limit}.run(jobs, work)` owns that bounded claim/join/rethrow protocol for independent jobs; the edge-driven compile scheduler remains specialized because provider readiness, not a flat input range, controls when its jobs may start. Both now delegate their identical thread-vector reserve/spawn/join lifecycle to `execution::run_workers`, while retaining their different queue and failure policies.
- ✅ `execution::run_workers` constrains the same invocation that `std::jthread` performs. Its by-value `Work` is already decayed and copied into each thread, so `std::invocable<Work>` checks the rvalue invocation required by the standard; the former `std::invocable<Work&>` admitted lvalue-only callables that failed later inside the library constructor.
- ✅ Compilation and Linking are the same two halves, and each section holds only what its phase uses. `link_executable` / `link_executables` already mirrored `compile_unit` / `compile_units`, but the Linking section also held members the phase does not own: `linkable_object_paths` moved beside `collect_module_ldflags`, the other shared projection over the unit list, while dependency-signature formatting, the loaded executable index and persistence belong to `cache::link_store`, and relink policy to `cache::analyzer` (same oracle as compile/std). Relink decisions are built by the same `filter | transform | ranges::to` pipeline as compile decisions instead of a push_back loop.
- ✅ Rebuild policy has one call surface. `cache::analyzer::rebuild_reason_for` covers project TUs, `std`, and links; `object_store` / `link_store` / `standard_module_store` keep persistence (and std hydrate/publish). `build_system` asks the analyzer and no longer reads rebuild decisions off the stores.
...
- ✅ One link-signature format instead of two. `compute_link_signature` and `compute_test_runner_signature` each built the same four-field flag tail (`|flags=|link=|modules=|imports=`) by hand, so a fifth field would have had to be added twice or the two caches would have disagreed about what invalidates them. `cache::link_store::link_signature(input_paths, import_flags)` owns the format and an owned pre-serialized common flag tail; each caller supplies only its input list and import flags. The test-object list was likewise spelled out twice (argv builder, signature) with the `is_test and not has_main` predicate written a third time in `collect_test_module_ldflags`; `test_units()` and `test_object_paths()` hold it once. The format now ends in `|format=cb-link-v2`: the deliberate one-time relink marks that test-unit import flags recorded by the signature are also present in the linker argv.
- ✅ A link decision cannot describe one command and execute another. Ordinary decisions retain the exact input-path and import-flag lists used to form their signature, while the test-runner path computes those lists once; `perform_link` accepts those ingredients and constructs `link_argv` itself instead of accepting an independently assembled command. This also fixed a latent test-runner bug: module linker flags contributed by test units invalidated the cache but were never passed to Clang, and its signature listed test/shared objects in the opposite order from the command.
...
- ✅ Header freshness uses component-aware source/PCM-root containment rather than raw prefix matching, so a sibling such as `/work/app-copy` is no longer treated as part of `/work/app`. `canonical_path` now uses `weakly_canonical` with an `error_code` and a lexically normalized absolute fallback: missing tails are normalized without a blanket `catch(...)` swallowing unrelated C++ exceptions.
- ✅ `output::step_state::failed()` establishes `ok == false` as well as replacing diagnostics. Its callers currently fail before reporting success, but the state transition now enforces its own contract instead of depending on call order.
- ✅ Source-suffix recognition computes the longest matching suffix with `std::ranges::max_element`; `.test.c++m` can no longer silently become `.c++m` if `supported_suffixes` is reordered.
- ✅ The empty `toolchain::driver` placeholder no longer pretends to be an implemented abstraction. Toolchain-owned policy began as direct `module_link_flags`, `module_compilation`, and `linkage` types; the later concrete implementation is named `clang_driver`, not a generic interface that falsely promises another compiler.
- ✅ `build_system` has one semantic settings boundary instead of a nine-parameter constructor followed by three setters. `cli::options::build_settings()` projects only build inputs; output selection, actions and test-runner forwarding remain in the CLI layer. The build system applies orchestration defaults and hands compiler-facing values to the selected concrete driver, while `main` is reduced to constructing the system from that value.
- ✅ Clang command policy has one concrete owner. `toolchain::clang_driver` discovers LLVM, owns compiler identity and Clang/libc++ flags, maps semantic module/BMI pairs to `-fmodule-file=`, and returns compile/link argv. Its compile-step callback states which step writes the BMI, so `build_system` can execute and report steps, publish provider readiness, and persist the std profile without encoding one- versus two-phase command sequences — argv is moved into each step, with no intermediate plan vector. Project compiles take `source::translation_unit` (same file, dependency order); `std` keeps a path-based entry point. Construction is a factory (`clang_driver::make`): the driver emits compiler-lookup and `--version` argv, `process::runner` executes them, and the returned driver is immutable — no injected availability callback. Compiler `--version` text still lives on `build_system` after the stamp probe. There is no virtual base until a working second compiler proves the common interface.
- ✅ Artifact naming has one build-tree boundary. `toolchain::artifact_conventions` exposes program-lifetime extension views selected by the concrete driver (`.pcm`, platform object and executable conventions), while `build_tree::paths` owns the `build-<os>-<config>/{pcm,obj,bin,cache}` tree, collision-preserving source/module stems, and depfile/diagnostic sidecars. `build_system` now consumes those paths instead of deriving filenames, and the shared std cache reuses the configured std artifact filenames rather than hard-coding `.o`.
- ✅ Reasons about a module artefact are the third named projection out of a translation unit, beside `compile_unit_of` and `source_unit_of`. `own_bmi_missing`, `own_bmi_stale`, `bmi_stale` and `dependency_bmi_stale` all name the same three things about the unit whose module triggered them — `module`, `bmi_path`, `full_path` — which was written out six times as a four-line designated initializer, once per site, with the unit's name (`tu`, `interface`, `dep_tu`) repeated three times inside it. `bmi_rebuild(kind, unit)` says it once, and each site is one line. The two blocks that fill in a reason inherited from a dependency are `attributed_to(reason, unit)`, which also settled that they differed only in a guard that was a no-op.
- ✅ The object-cache load carries the diff as the miss, not a reason beside a diff. The intermediate `object_cache_load` held `std::optional<rebuild_kind> miss_reason` and a bare `object_cache_profile_diff` documented in prose as "filled for `profile_change`" — the coupling the type could have stated instead. The optional was a `bool` in disguise because `profile_change` was its only possible value. `cache::object_store` now owns one `std::optional<object_cache_profile_diff> profile_change`: engaged means the blanket miss happened and holds the field that caused it; the analyzer reads that state directly from the store.
- ✅ `transitive_bmi_newer_than_object` returns the reason. It used to return `bool` and assign to a `rebuild_info&` out-parameter that meant nothing when the bool was false, so the caller had to declare an empty reason before asking the question — the same hidden channel as the object-cache members, in argument form. `visited` stays a parameter: it is the walk's own state, not an answer travelling back.
...
- ✅ `test_runner.c++` is a non-module TU (`import tester;`) rather than a `tester` implementation unit that owned `main` (and needed `-Wmain-attached-to-named-module`). Partition imports are only legal inside the module, so the primary interface re-exports `:console_observer`, `:jsonl_observer`, and `:junit_observer` for the composition root.
...
- ✅ Assertion accounting is atomic on `test_statistics` (`record_assertion` / `begin_assertion` / `complete_assertion`). Soft asserts from test-spawned threads that share a context can no longer lose increments. Under `--jobs>1`, orphans that still land on the run-wide fallback are reified as `<unattributed thread assertion>` so `failed_test_ids` is never empty when `passed` is false. Parallel waves drop filtered-out cases before chunking so they cannot steal `--jobs` slots. Pinned by `[self]` concurrent-count and orphan-attribution probes.
- ✅ Compiler probe and version stamp go through `process::runner`. A bare `LLVM_CXX` / `CXX` name was interpolated into `command -v …` and `clang++ --version` called `std::system` with a hand-built redirect — the two places that bypassed the single shell boundary. Both now use `process::runner::invoke_shell`: the lookup is a quoted `sh -c 'command -v …'`, and the version line is captured to `cache/compiler-version.txt` like every other child command.
...
- ✅ Makefile CI lane (`makefile-build-and-test` in `.github/workflows/ci.yml`): matrix
  `debug` / `release` — `make tests` (fail on any compiler `warning:`), then
  `make run_tests` with `--tags=[self]`. CB also surfaces warnings from units it
  compiles (`diagnostics` on successful `compile_end` / `link_end` / `command_end` in
  failures mode), so Make is no longer the only place `-Wall` noise is visible — it
  remains the full-rebuild cross-check beside CB.
- ✅ CMake + Ninja CI lane (`cmake-ninja-build-and-test` in `.github/workflows/ci.yml`): configure and build [`CMakeLists.txt`](../CMakeLists.txt) for `debug` and `release` (same matrix spelling as CB/Make; mapped to CMake's `Debug`/`Release` build type), fail on any compiler `warning:`, then `[self]` sequentially and at `--jobs=$(nproc)`, the standalone suite, and the `run_tests` target. CMake is pinned via `.github/actions/setup-cmake` so the lane fails on our changes rather than on a new CMake, and the dev container installs the same pin so the third path is runnable where it is documented. The lane found a real portability bug on its first run: `CMakeLists.txt` originally let CMake supply `std`, which it locates with `clang++ -print-file-name=libc++.modules.json`, and apt.llvm.org's `libc++-21-dev` installs a second copy of that manifest under `/usr/lib/<triple>` whose relative `source-path` resolves only from `${LLVM_PREFIX}/lib`. Clang answered with the broken copy and every Ubuntu configure died on a missing `/lib/share/libc++/v1/std.cppm`. The fix follows `config/compiler.mk`: compile `std` from `${LLVM_PREFIX}/share/libc++/v1/std.cppm` as an ordinary module target and never ask for the manifest.
