...
- ✅ `rebuild_reason` on `compile_end` when `cache_hit:false` (short kind: `not_in_cache`, `source_stale`, `header_stale`, `header_missing`, `depfile_unusable`, `pcm_stale`, `dependency_pcm_stale`, `profile_change`, …).
...
- ✅ Clang command policy has one concrete owner. `toolchain::clang_driver` discovers LLVM, owns compiler identity and Clang/libc++ flags, maps semantic module/BMI pairs to `-fmodule-file=`, and returns compile/link argv. Its compile plans also state which step writes the BMI, so `build_system` can execute and report steps, publish provider readiness, and persist the std profile without encoding one- versus two-phase command sequences. The driver receives small path/module request values rather than `source::translation_unit`, and there is no virtual base until a working second compiler proves the common interface.
- ✅ Artifact naming has one layout boundary. `toolchain::artifact_conventions` exposes program-lifetime extension views selected by the concrete driver (`.pcm`, platform object and executable conventions), while `layout::paths` owns the `build-<os>-<config>/{pcm,obj,bin,cache}` tree, collision-preserving source/module stems, and depfile/diagnostic sidecars. `build_system` now consumes those paths instead of deriving filenames, and the shared std cache reuses the configured std artifact filenames rather than hard-coding `.o`.
