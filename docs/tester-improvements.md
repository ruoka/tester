...
- 📋 Assertion accounting is still not thread-safe inside a single `execution_context`. Parallel workers each own a context, and the console/JSONL locks make buffers safe, but counters are plain integers — concurrent assertions from test-spawned threads (or orphan soft fails under `--jobs>1`) can lose counts. Either say in the docs that reporting is single-threaded per context, or make the counters atomic and pin the parallel soft-fail case (draft [#56](https://github.com/ruoka/tester/pull/56)).
...
- ✅ Scanner and inventory smoke cases stop at `list`. Fifteen of the 45 CB smoke cases asked questions `scan_and_order` already answers — dotted modules, GMF preambles, commented and spliced phantoms, dead `#if` arms, deps skipping, basename and reserved-`std` refusals, `compile_commands` inventory — and then compiled the fixture "to prove it still builds." Those builds are gone; the collision cases call `list` too, since the refusal is thrown before any compile. The suite is 254 checks (was 247 before `header_missing`; ~61 s class). The remaining rebuild/link/warning/test cases still need a toolchain; unit-testing `splice_physical_lines` still needs the extraction below.
