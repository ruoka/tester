...
- ✅ Container element comparisons use the same mathematical-value rule. `check_contains` / `require_contains` (container overload) went through `std::ranges::contains`, and `check_container_eq` compared with language `==`, so `check_contains(std::vector<std::size_t>{npos}, -1)` and `check_container_eq(std::vector<int>{-1}, std::vector<unsigned>{UINT_MAX})` both reported equal — the wraparound the scalar matchers had already fixed. Element equality now routes promotable integers through `std::cmp_equal` after unary `+`; equal values across signedness still match (`require_contains(vector<size_t>{3}, 3)`).
