...
- ✅ Class-based subsystem boundaries now divide the single `cb.c++`: `cb::source::translation_unit`
  ...
  `cb::cache::storage_file`; `object_store`, `link_store`, `standard_module_store`, and
  `compiler_stamp` own cache maps/signatures, persistence, status, and cache-specific reuse
  decisions. `cb::process::runner` privately owns shell quoting/argv joining as well as spawning,
