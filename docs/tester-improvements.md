...
- ✅ The preamble cleaner follows the three constructs that outlive their line. A backslash-newline splice continues a string literal and a `//` comment alike — translation phase 2 runs before either is recognised, and clang confirms the second with `-Wcomment` — and a raw string has no escapes at all, so only `)delimiter"` ends it. All three branches stopped at `\n`, so each carried an `import` that is not code into the module graph. The raw string gets its own branch with the delimiter matched by backreference; the rest admit `\\\r?\n`. Branches are unrolled (a character-class run, then an escape group, then the run again) because the per-character alternation cost 0.19 s of `list` against 0.17 s unrolled from a 0.15 s baseline, and this pass has dominated scan time before.
...
- ✅ `emit_compile_start`, `emit_compile_end`, and `emit_link_end` are gone. They existed because the interface was positional: their whole body was mapping a unit onto unnamed arguments, including two copies of the `is_modular ? pcm_path : {}` projection that could have disagreed. Call sites now notify directly, as the other ~40 already did.
...
- 📋 The emit-once latches are still producer state. `emit_failed_build_end` guards on `current_phase` and `build_end_emitted`, and the tester side is worse: `publisher::print_test_statistics(std::optional<bool>&, bool&, std::string_view)` takes the runner's private bookkeeping by mutable reference so a free function can flip it. This is lifecycle logic, not formatting, so it belongs in one place that knows `build_end` and `run_end` are emitted exactly once — not duplicated into each observer, which could then disagree.
- 📋 Rebuild sentences are still built by the producer. `format_rebuild_message` / `format_link_message` compose human text like `"Linking X because input objects changed"` inside `cb.c++` and ship it in `rebuild_info.message`; both observers consume it, so a shared formatter they both call would keep the wire identical while moving presentation out of the build system and making the sentences testable.
- 📋 Seven of the tester publisher's functions are pure forwarders (`emit_output_eof`, `print_test_catalogue`, `print_test_cases`, `print_test_results`, `print_test_failures`, `test_case`, `exception`) and could be dropped for direct `notify`. The rest earn their place by building the event payload, which is the pattern the cb side just adopted.
