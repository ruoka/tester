...
- ✅ The empty `toolchain::driver` placeholder no longer pretends to be an implemented abstraction. Toolchain-owned concepts are direct types until there is a real driver: `toolchain::module_link_flags`, `module_compilation`, and `linkage` name compiler/linker policy without claiming command construction has moved out of `build_system`.
- ✅ `build_system` has one semantic settings boundary instead of a nine-parameter constructor followed by three setters. `cli::options::build_settings()` projects only build inputs; output selection, actions and test-runner forwarding remain in the CLI layer. The constructor now owns include-path-to-flag conversion, configuration defaults, toolchain module/linkage policy and worker-limit state, while `main` is reduced to constructing the system from that value.
