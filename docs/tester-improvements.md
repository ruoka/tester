...
- 🔶 Class-based subsystem boundaries now divide the single `cb.c++`: `cb::discovery::source_scanner`
  owns collection, parsing, dependency edges and topological order; `cb::cache::staleness` owns
  recursive object/BMI/header freshness; and `cb::cli::parser` / `options` own strict argv parsing
  and test-runner forwarding. No `*.h++` split is required. Cache persistence and shared-std
  storage still live on `build_system`, so the cache-layer extraction remains partial.
...
- 📋 Extract `translation_unit` and `parse_translation_unit` into their own header. Independent of the observer work: they are a self-contained island in a ~3,300-line file, and extracting them is what would let the preamble scanner be tested directly instead of only through `smoke.sh` fixture directories.
