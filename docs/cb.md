...
- **Module graph** — scans source preambles for `import` / `export module` and builds a dependency graph (no `clang-scan-deps` in `cb.c++`, which keeps scanning compiler-independent). Comments, string literals and `#if 0` bodies are stripped before matching, so a commented-out `import` does not become a graph edge; genuine `#ifdef` branches are over-approximated (every branch contributes an edge) because resolving them would require a preprocessor
