...
If the depfile is missing, unreadable, or malformed (no `:`), CB rebuilds with `rebuild_reason: "depfile_missing"` and the `.d` path in `rebuild.trigger_path` — it must not treat “no parseable prerequisites” as a cache hit (that silently skips header edits after upgrades or lost `*.d` files).
...
- `rebuild_reason: "depfile_missing"` — compiler `.d` missing, unreadable, or malformed; the `.d` path is in `rebuild.trigger_path`
