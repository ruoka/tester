...
- **Module graph** — scans source preambles for `import` / `export module` and builds a dependency graph (no `clang-scan-deps` in `cb.c++`, which keeps scanning compiler-independent). Continued lines are spliced first, as translation phase 2 does (raw-string bodies excepted, matching [lex.pptoken] reversion), and then comments, string literals and `#if 0` bodies are stripped before matching, so a commented-out `import` does not become a graph edge and a `#if \` / `0` region is as dead here as it is to the compiler; genuine `#ifdef` branches are over-approximated (every branch contributes an edge) because resolving them would require a preprocessor
